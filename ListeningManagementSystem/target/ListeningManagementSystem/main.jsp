...
var sysBBS = "<span style='font-size:14px; line-height:30px;'>欢迎光临在线交流讨论室，请遵守聊天室规则，不要使用不文明用语。</span><br><span style='line-height:22px;'>";var sysBBS = "<span style='font-size:14px; line-height:30px;'>欢迎光临心之语聊天室，请遵守聊天室规则，不要使用不文明用语。</span><br><span style='line-height:22px;'>";
	...
	function showOnLine(){
		// 异步发送请求 获取在线人员列表
		// Jquery发送异步请求
		$.post("${pageContext.request.contextPath}/onlineUI?"+new Date().getTime(),function(data){
	...
	function showContent(){
		$.post("${pageContext.request.contextPath}/getMessage?"+new Date().getTime(),function(data){
			$("#content").html(sysBBS+data);
		});
	}
	
	function set(selectPerson){	//自动添加聊天对象
		if(selectPerson != "${existUser.stuName}"){
	...
	function send(){
		...
		$.post("${pageContext.request.contextPath}/sendMessage?"+new Date().getTime(),$("#form1").serialize(),function(data){
	...
	<table width="778" height="276" border="0" align="center"
		...
		<tr>
			...
			<td width="613" height="200px" valign="top"
				...
				style="padding:5px; ">
				<div style="height:290px; overflow:auto" id="content">聊天内容</div></td>
	...
	<table width="778" height="95" border="0" align="center"
		...
		<form action="" id="form1" name="form1" method="post">
			...
			<tr>
				...
				<input name="from" type="hidden" value="${existUser.stuName}">[${existUser.stuName} ]对
