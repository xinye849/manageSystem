    ...
    $(function () {
                ...
                $.get('${pageContext.request.contextPath}/findStudentListensByClassRoom',
                    {classRoom:'${existUser.classRoom}'},function (data) {
                        ...
                        $('<thead><tr><th>课程</th><th>节数</th><th>日期</th><th>讲课老师</th><th>听课老师</th><th>听课地点</th></tr></thead>')
                            .appendTo('#tb');
                        $.each(eval(data),function () {
                            $(' <tbody><tr><td>'+this.listenClassCourse+'</td><td>'+this.listens+'</td><td>'+this.listenClassDate+'</td><td>'+this.listenClassLectureTeacher+'</td><td>'+this.listenClassTeachers+'</td><td>'+this.classRoom+'</td></tr></tbody>').appendTo('#tb');
                        })
                    });
